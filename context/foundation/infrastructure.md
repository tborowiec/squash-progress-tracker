...
- **Region ↔ AI-provider API RTT eats the <5s parse NFR.** Render's region is fixed per service and non-edge; if the service region is far from the chosen AI provider's API region, network round-trips silently consume the latency budget the PRD's 5-second parse target depends on. (The provider — e.g. Anthropic, Google Gemini, OpenAI — is **not yet decided**; see Out of Scope.)
...
- **Secrets**: Stored as Render environment variables / Env Groups (and secret files) in the dashboard or via the `render` CLI / blueprint — never committed. The app reads the AI provider's API key (a placeholder `LLM_API_KEY` — the specific provider is deferred; see Out of Scope) and the DB credentials (`SPRING_DATASOURCE_URL` / `_USERNAME` / `_PASSWORD`, mapped from Render's `DATABASE_URL`) from the environment. Rotation = set the new value in the dashboard and redeploy.
...
- **AI/LLM provider selection — deliberately deferred.** The product needs an LLM for text parsing (FR-003) and game-plan generation (FR-010), but the *provider* (Anthropic, Google Gemini, OpenAI, …) and integration path (Spring AI vs a direct vendor SDK) are an open decision. This document is provider-agnostic: it refers to a generic `LLM_API_KEY` and "the AI-provider API." Two infra-relevant points carry forward whichever provider wins: (1) the provider's API region affects the <5s parse NFR (co-locate the Render region with it), and (2) the provider's API key is a human-rotated secret. Rename `LLM_API_KEY` once the provider is chosen.
