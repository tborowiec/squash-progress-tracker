...
| container smoke | none yet — see §3 Phase 5 | — | build + run the Docker image in CI; HTTP-assert health + SPA root + `401` on `GET /api/auth/me`. Deterministic, no browser. Closes #6 (the running artifact, not just the build) |
...
| build + run + HTTP-smoke the Docker image | CI on PR | required after §3 Phase 5 | deploy parity (#6) — the *running* artifact: missing frontend, `$PORT` bind, discrete `DB_HOST`/`DB_PORT`/`DB_NAME`/`DB_USER`/`DB_PASSWORD` mapping, startup/OOM failures that a build-only gate never exercises |
...
- Phase 5 complete. Container smoke (`smoke-container.sh`): builds the image, runs it against a throwaway Postgres on a shared Docker network, asserts `/actuator/health` (200 + UP), SPA root (200 + `<div id="root">`), and `GET /api/auth/me` (401). DB wired via discrete `DB_HOST`/`DB_PORT`/`DB_NAME`/`DB_USER`/`DB_PASSWORD` vars. Browser e2e (`log-match.spec.ts`): one Playwright critical-flow happy-path (login → log match → history) against the running image, parse mocked at the browser edge. Both gate `ci-success` in CI.
