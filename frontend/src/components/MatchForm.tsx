...
import { CreateOrUpdateMatchRequest, SetScoreRequest } from '../api/matches'
import { ApiError } from '../api/auth'
import { AxiosError } from 'axios'

export const today = () => new Date().toISOString().split('T')[0]

export interface SetRow { playerScore: string; opponentScore: string }
...
export default function MatchForm({ initial, onSubmit, submitLabel }: Props) {
  ...
  const [sets, setSets] = useState<SetRow[]>(initial?.sets ?? [{ playerScore: '', opponentScore: '' }])
