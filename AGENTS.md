...
**Frontend (React/TypeScript)** is formatted and linted with **Biome** (config in `frontend/biome.json`: single quotes, 2-space indent, no semicolons, 100-col). Same two-layer setup: (1) a per-edit Claude Code hook (`.claude/hooks/format-frontend.sh`) runs `biome check --write` on each `.ts/.tsx/.css/...` file on Write/Edit — formatting + *safe* lint fixes (~15ms), surfacing unfixable lint errors back to the agent; (2) a lefthook pre-commit gate (`lefthook.yml`: `biome-check` + `frontend-typecheck`) blocks staged frontend files on formatting violations, lint errors, or `tsc --noEmit` type errors. Biome is an `npm` devDependency, so `npm ci` in `frontend/` is all the setup needed. Manual commands from `frontend/`: `npm run format`, `npm run lint`, `npm run typecheck`. Advisory lint warnings (`noNonNullAssertion`, `noArrayIndexKey`) don't block.
