...
public class GamePlanPromptBuilder {
    ...
    private String buildUserMessage(String opponentName, List<Match> matches) {
        ...
        for (int i = 0; i < matches.size(); i++) {
            var m = matches.get(i);
            var setsWon = (int) m.getSets().stream().filter(s -> s.getPlayerScore() > s.getOpponentScore()).count();
            var setsLost = (int) m.getSets().stream().filter(s -> s.getPlayerScore() < s.getOpponentScore()).count();
            var result = setsWon > setsLost ? "WON" : (setsLost > setsWon ? "LOST" : "DRAW");

            sb.append("\nMatch ").append(i + 1).append(": ").append(m.getMatchDate())
              .append(" — ").append(result)
              .append(" (").append(setsWon).append("–").append(setsLost).append(" sets)\n");

            for (var set : m.getSets()) {
                sb.append("  Set ").append(set.getSetNumber()).append(": ")
                  .append(set.getPlayerScore()).append("–").append(set.getOpponentScore()).append("\n");
            }
