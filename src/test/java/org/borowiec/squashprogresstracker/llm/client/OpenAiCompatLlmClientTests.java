...
class OpenAiCompatLlmClientTests {
    ...
    @BeforeEach
    void setUp() {
        var builder = RestClient.builder().baseUrl(BASE_URL).defaultHeader("Authorization", "Bearer test-key");
    ...
    void generate_happyPath_returnsChoiceContentAndSendsBearerHeader() {
        mockServer
                ...
                .andExpect(bodyJson(
                        json -> assertThat(json.path("model").asString()).isEqualTo("test-model")))
    ...
    void generateStructured_sendsJsonSchemaWithStrictTrue_andDeserializes() {
        mockServer
                ...
                .andExpect(bodyJson(json -> {
                    ...
                    assertThat(json.path("response_format").path("type").asString())
                            .isEqualTo("json_schema");
                    assertThat(json.path("response_format")
                                    .path("json_schema")
                                    .path("strict")
                                    .asBoolean())
                            .isTrue();
    ...
    void generate_http4xx_throwsLlmExceptionWithStatus() {
        mockServer.expect(requestTo(COMPLETIONS_URL)).andRespond(withStatus(HttpStatus.UNAUTHORIZED));
    ...
    void generate_http5xx_throwsLlmExceptionWithStatus() {
        mockServer.expect(requestTo(COMPLETIONS_URL)).andRespond(withServerError());
    ...
    void generate_emptyChoices_throwsLlmException() {
        ...
        assertThatThrownBy(() -> client.generate(LlmRequest.ofUser("test"))).isInstanceOf(LlmException.class);
    }
    ...
    void generate_nullContent_throwsLlmException() {
        mockServer
                .expect(requestTo(COMPLETIONS_URL))
                .andRespond(
                        withSuccess("{\"choices\":[{\"message\":{\"content\":null}}]}", MediaType.APPLICATION_JSON));
    ...
    void generateStreaming_malformedChunk_throwsLlmException() {
        ...
        mockServer.expect(requestTo(COMPLETIONS_URL)).andRespond(withSuccess(sseBody, MediaType.TEXT_EVENT_STREAM));
    ...
    void parseSseStream_onTokenFiresInOrder() throws Exception {
        ...
        var tokens = new ArrayList<String>();
        OpenAiCompatLlmClient.parseSseStream(new BufferedReader(new StringReader(sseBody)), tokens::add, objectMapper);
